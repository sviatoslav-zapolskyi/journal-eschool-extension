...
if (location.hostname && location.hostname.endsWith('journal.eschool-ua.com')) {
  setTimeout(() => {
    try { handlers.FETCH_PROFILE && handlers.FETCH_PROFILE(); } catch (e) { /* ignore */ }
  }, 200);
}
...
handlers.FETCH_PROFILE = () => {
  ...
  fetch('https://api.eschool-ua.com/profile', {
    ...
    .then(profile => {
      try { localStorage.setItem('__cached_profile__', JSON.stringify(profile)); } catch (e) {}
      ...
      if (partsHref) {
        fetch(partsHref, { headers: { 'api-key': apiKey }, credentials: 'include' })
          .then(r => r.json())
          .then(partsResp => {
            try { localStorage.setItem('__cached_parts__', JSON.stringify(partsResp)); } catch (e) {}
    ...
    .finally(() => { isFetchingProfile = false; });
};
